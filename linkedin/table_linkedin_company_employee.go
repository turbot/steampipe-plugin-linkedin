...
func tableLinkedInCompanyEmployee(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		Name:        "linkedin_company_employee",
		Description: "Search employees for a given company on LinkedIn.",
		List: &plugin.ListConfig{
			Hydrate: listCompanyEmployee,
...
func listCompanyEmployee(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {

	conn, err := connect(ctx, d)
	if err != nil {
		plugin.Logger(ctx).Error("linkedin_company_employee.listCompanyEmployee", "connection_error", err)
		return nil, err
	}
	...
	if err != nil {
		plugin.Logger(ctx).Error("linkedin_company_employee.listCompanyEmployee", "query_error", err, "companyID", companyID, "query", query)
		return nil, err
	}
